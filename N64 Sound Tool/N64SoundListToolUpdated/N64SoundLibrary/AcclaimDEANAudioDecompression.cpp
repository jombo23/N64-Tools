...
void FUN_00008c38(unsigned char* RAM, unsigned long param_1, int param_2)
{
  ...
  if (sVar1 == 2) {
    ...
    if (0 < sVar5) {
      ...
      do {
		  uVar2 = CSharedFunctions::CharArrayToLong(RAM, puVar12);
        iVar6 = (uVar2 & 0xff) * 0x10 + -0x7ffb8530;
    ...
    if (0 < sVar5) {
      ...
      do {
		  uVar2 = CSharedFunctions::CharArrayToLong(RAM, puVar12);
        iVar6 = (uVar2 & 0xff) * 0x20 + -0x7ffb7530;
...
bool CAcclaimDEANAudioDecompression::DecompressMARKSound(unsigned char* ROM, int romOffset, int compressedSize, CString outputFilename, float samplingRateFloat)
{
	int sizeRAM = 0x800000;
	unsigned char* RAM = new unsigned char[sizeRAM];
	for (int x = 0; x < sizeRAM; x++)
	...
	CSharedFunctions::WriteShortToBuffer(RAM, 0x83904, 0x0036);
	CSharedFunctions::WriteLongToBuffer(RAM, 0x00083878, 0x00000000);
	CSharedFunctions::WriteLongToBuffer(RAM, 0x0008387C, 0x00070004);
	...
	// Length
	CSharedFunctions::WriteLongToBuffer(RAM, 0x00083908, compressedSize / 0x1C);
