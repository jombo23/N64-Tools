...
bool DLSFile::SaveDLSFile(const TCHAR* filepath)
{
	vector<BYTE> dlsBuf;
	WriteDLSToBuffer(dlsBuf);
	return UI_WriteBufferToFile(filepath, &dlsBuf[0], dlsBuf.size());
}
...
bool DLSFile::UI_WriteBufferToFile(const TCHAR* filepath, unsigned char* buf, unsigned long size)
{	
...
void DLSInstr::Write(vector<BYTE> &buf)
{
	...
	theUINT = GetSize() - 8;
	WriteLIST(buf, 0x696E7320, theUINT);				//write "ins " list
	...
	WriteLIST(buf, 0x6C72676E, theUINT);				//write the "lrgn" list
	for (UINT i = 0; i < aRgns.size(); i++)
...
{
	WriteLIST(buf, 0x72676E32, (UINT)(GetSize() - 8));	//write "rgn2" list
...
{
	WriteLIST(buf, 0x6C617232, GetSize() - 8);					//write "lar2" list
...
void DLSArt::AddVibrato(long frequency, long delay_time)
{
	aConnBlocks.insert(aConnBlocks.end(), new ConnectionBlock(CONN_SRC_NONE, CONN_SRC_NONE, CONN_DST_VIB_FREQUENCY, CONN_TRN_NONE, frequency));
	aConnBlocks.insert(aConnBlocks.end(), new ConnectionBlock(CONN_SRC_NONE, CONN_SRC_NONE, CONN_DST_VIB_STARTDELAY, CONN_TRN_NONE, delay_time));
}
...
void DLSWave::Write(vector<BYTE> &buf)
{
	...
	WriteLIST(buf, 0x77617665, GetSize()-8);			//write "wave" list
